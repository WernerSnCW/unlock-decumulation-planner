...
- **Frontend Features:**
    ...
    - **EIS Strategy**: Models annual EIS/SEIS allocation programme. Two allocation modes: **Fixed** (manual EIS/SEIS amounts) and **Tax Allowance** (auto-calculates maximum EIS/SEIS needed to zero out estimated income tax — SEIS fills first up to £200k cap at 50% relief, then EIS at 30% relief — with a configurable percentage slider 10-100% of that allowance). **Quality tiers**: cautious (2.9× base), base (5.65× base), strong (10.25× base) with bear/base/bull/worst-case scenario matrix giving different multiples. **Scheme types**: EIS only, SEIS only, or Blend (13% SEIS / 87% EIS auto-split). **Growth mode**: flat (same each year) or +5% pa increasing investment. Scenario selector: Bear, Base Case, Bull, All Fail. **Exit ramp**: portfolio value builds gradually over 7 years (0%→5%→15%→30%→50%→70%→85%→100% of target 3.8× multiple) rather than instant growth. **Relief capping**: income tax relief capped at estimated tax liability — cannot claim more relief than you owe. **Loss relief at marginal rate**: worst-case net cost uses `calculateMarginalRate()` (PA trap → 40%, else marginal capped at 45%) instead of fixed constants; displayed as "Xp/£" in summary. **Investment phase vs harvest phase**: configurable investment_years (0 = continuous); new capital only deployed during investment phase, existing cohorts continue maturing during harvest phase. **CGT deferral**: model capital gain events (year, gain, rate: auto/18%/24%) that can be deferred by investing in EIS; auto rate uses 24% if marginal ≥ 40%, else 18%; cumulative deferral tracked and displayed in amber. Engine deducts allocation from cash/liquid assets first, tracks cohorts for BPR (IHT-exempt after 2 years). Summary panel shows annual allocation, relief, effective cost, total invested, EIS portfolio value, net cost if all fail (with p/£), IHT-exempt amount, CGT deferred, phase breakdown. In Tax Allowance mode, shows estimated income tax, max SEIS/EIS to zero tax, and amounts at selected percentage. Portfolio chart shows EIS Programme as separate area + dashed comparison line for alternate scenario. Dual simulation runs both scenarios for comparison.
    - **VCT Strategy**: Models annual VCT investment programme with 5-year hold cycle. 20% income tax relief (30% for pre-2026 subscriptions), tax-free dividends (~5% pa), CGT-free disposal. At 5-year liquidation: "Reinvest" recycles proceeds into new VCTs for fresh relief (total subscribed shown), or "Cash Out" returns proceeds to cash in portfolio. Scenario toggle: Base Case (0.5% pa growth) vs -50% Loss. Worst case affects liquidation values and dividend base. VCTs NEVER qualify for BPR/IHT relief. Summary shows fresh capital, total subscribed (if recycling), relief, dividends, VCT portfolio value, proceeds returned (if cashing out). Purple area on portfolio chart.
