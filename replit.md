...
- **Components** (`src/components/`):
  ...
  - `AssetEditor` — modal for editing asset register values (current value, growth rate, income, acquisition cost, mortgage). Header button, unsaved changes guard, per-asset and global reset
  - `DisclosurePanel` — collapsible assumptions and disclosure text
- **Strategy Mechanisms**: 6 boolean toggles in sidebar (preserve_eis_bpr, preserve_aim_bpr, preserve_vct_income, draw_isa_early, draw_pension_early, protect_property) that influence `scoreAssetForDrawdown` scoring. Grouped under "IHT Reduction" and "Tax / Income" headers.
- **Asset Editor**: Modal dialog (button in header) lets users override key asset values (current_value, assumed_growth_rate, income_generated, acquisition_cost, mortgage_balance). Uses asset_id-based lookup for reset/override detection. Shows "was £X" indicators for changed values. Unsaved changes prompt on close. "Reset to default" per asset and globally. Header button shows "Assets (edited)" when overrides active. Simulation re-runs immediately on apply.
